...
class PetWindow(QtWidgets.QLabel):
    ...
    def analyze_screen_and_comment(self, force=False):
        ...
        try:
            ...
            screenshot = self._capture_screen_hidden()

            self.move(original_pos)
            self.show()

            description = self._analyze_image(screenshot)
            if not self._validate_description(description):
                return

            data = self._generate_comment(description)
            if not self._validate_language_japanese(data):
                return
    ...
    def _capture_screen_hidden(self):
        import time
        self.hide()
        self.subtitle.hide()
        if self.chat_input.isVisible():
    ...
    def _analyze_image(self, screenshot):
    ...
    def _generate_comment(self, description):
        import requests
...
この画面を見て、めぐるとして一言だけ自然に反応して。短めにね。"""
    ...
    def _validate_language_japanese(self, data):
        import re
        subtitle_text = data.get("subtitle_zh") or data.get("text") or ""
        ...
        japanese_chars = len(re.findall(r'[\u3040-\u30ff\u4e00-\u9fff]', subtitle_text))
        if total_chars > 0 and japanese_chars / total_chars < 0.3:
            print(f"[Vision] Response not in Japanese/Kanji ('{subtitle_text}'), skipping.")
            return False
        return True
